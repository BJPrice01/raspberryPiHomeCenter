...
#  
#  Small modifications for speed of response made by Brennan Price
...
send_to_plug() {
   ...
   port="$2"
   payload="$3"
   echo -n "$payload" | base64 --decode | nc -w 1 $ip $port || echo couldn''t connect to $ip:$port, nc failed with exit code $?
}
...
cmd_print_plug_relay_state(){
   output=$(send_to_plug $ip $port "$payload_query" | decode | grep -oa 'relay_state":[0,1]' | grep -o '[0,1]')
   if [[ $output -eq 0 ]]; then
     echo OFF
